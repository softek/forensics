...
NamedSet.exe reads from STDIN a list of lines and generates
...
dir /on /b *.exe *.dll|NamedSet.exe

# Example 2
Generate a name for the scpview.exe and its dependencies.

deps.exe scpview.exe|FileInfo --sha1|NamedSet.exe
"))
...
(defn run [lines]
  (let [content (get-hash-content-version-0 lines)
    ...
    (println (str "Vordsion:\t" algorithm-version "_" words))
    ...
    (println "-----End Content-----")
    (println)
    (println "Generated by Vordsion algorithm version" algorithm-version)))
